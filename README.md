...
## การเลือกโมเดล: qwen3:4b vs qwen3:1.7b

ระบบใช้ `qwen3:4b` เป็นโมเดลหลัก แต่มีบางหน้าที่ต้องสลับเป็น `qwen3:1.7b` ชั่วคราว — นี่คือเหตุผล

### ปัญหาของ qwen3:4b — "ตอบว่าง" บนหน้าซับซ้อน

`qwen3:4b` เป็น reasoning model ที่ "คิด" ผ่าน `<think>...</think>` ก่อนตอบ
เมื่อหน้าเอกสารซับซ้อน เช่น หน้า 34–36 ที่มีรหัสวิชาแบบ `xxxxxxx` และเงื่อนไข "หรือ"
โมเดลใช้ token budget (8,192) หมดไปกับการคิด จนไม่เหลือโควตาผลิตคำตอบ → **content ว่าง 0 ตัวอักษร**

```text
หน้า 34, 35, 36 — ล้มเหลวครบทั้ง 3 ครั้ง (เต็มโควตา retry ของสคริปต์)
```

ตัวแปรที่ปรับไม่ได้:
- `num_predict` เป็นค่าคงที่ในโค้ด ปรับผ่าน env ไม่ได้
- `LAB7_CHUNK` ลดต่อไปไม่ได้ (อยู่ที่ 1 หน้า/ก้อนอยู่แล้ว)

→ ตัวแปรเดียวที่เหลือตามกรอบใบงาน (หัวข้อ 6.5) คือ **ขนาดโมเดล**

### ผลของ qwen3:1.7b บนหน้าที่ 4b ตอบว่าง

```text
หน้า 34, 35, 36 — สำเร็จทั้ง 3 หน้าในครั้งแรก ไม่มี "ตอบว่าง" เลย
ความเร็ว: ~30–36 วินาที/หน้า  (เทียบ qwen3:4b ~400–460 วินาที/ครั้ง)
```

### Trade-off ที่บันทึกไว้

| | qwen3:4b (โมเดลหลัก) | qwen3:1.7b (fallback) |
|---|---|---|
| ความเร็ว | ช้า (~400–460 วิ/หน้า) | เร็ว (~30–36 วิ/หน้า) |
| Hallucination | ❌ ไม่เคยเกิด | ✅ เกิด (แต่งวิชาปลอม 6 รายการ) |
| ความเที่ยงตรงภาษาไทย | สูง | ต่ำกว่า (สระ/วรรณยุกต์หลุดบ้าง) |
| ลักษณะความผิดพลาด | "เงียบ" (ตรวจจับง่าย) | "เนียน" (ข้อมูลปลอมดูสมเหตุสมผล) |

> **สรุป:** ใหญ่ (4b) ล้มเหลวแบบ "เงียบ" — ตรวจจับและข้ามได้ง่าย  
> เล็ก (1.7b) เร็วกว่ามาก แต่ล้มเหลวแบบ "เนียน" — ข้อมูลเท็จที่ดูสมเหตุสมผล ตรวจจับยากกว่า

ดังนั้น `qwen3:1.7b` ถูกใช้เป็น **patch เฉพาะหน้าที่ 4b ตอบว่างซ้ำๆ เท่านั้น** ไม่ได้เปลี่ยนเป็นโมเดลหลัก
