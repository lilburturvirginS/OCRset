...
def clean_sql_output(s: str) -> str:
    """ตัด <think> และ fence ออกจาก SQL ที่โมเดลตอบมา รองรับทั้ง query บรรทัดเดียวและหลายบรรทัด"""
    s = re.sub(r"<think>.*?</think>", "", s, flags=re.S)
...
def ask(conn: sqlite3.Connection, question: str,
    ...
    ขั้นตอน: สร้าง SQL (Tier 1: MODEL_SQL) -> ตรวจ -> รัน -> สรุปเป็นภาษาไทย (Tier 2: MODEL_ANSWER)
    ...
    for attempt in range(2):
        try:
            raw_sql = ollama_generate(
                ...
                }, model=MODEL_SQL, num_ctx=4096, num_predict=256)
    ...
    raw_answer = ollama_generate(
        ...
        fmt={
            ...
            "required": ["answer"],
            "additionalProperties": False,
        }, model=MODEL_ANSWER, num_ctx=4096, num_predict=256).strip()
