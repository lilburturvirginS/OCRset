...
THAI_PUA_MAP = {
    0xf700: 0x0e48,  # ่ (MAI EK)
    0xf701: 0x0e34,  # ิ (SARA I)
    0xf702: 0x0e35,  # ี (SARA II)
    0xf703: 0x0e36,  # ึ (SARA UE)
    0xf704: 0x0e37,  # ื (SARA UEE)
    0xf705: 0x0e48,  # ่ (MAI EK shifted)
    0xf706: 0x0e49,  # ้ (MAI THO shifted)
    0xf707: 0x0e4a,  # ๊ (MAI TRI shifted)
    0xf708: 0x0e4b,  # ๋ (MAI CHATTAWA shifted)
    0xf709: 0x0e4c,  # ์ (THANTHAKHAT shifted)
    0xf70a: 0x0e48,  # ่ (MAI EK)
    0xf70b: 0x0e49,  # ้ (MAI THO)
    0xf70c: 0x0e47,  # ็ (MAITAIKHU)
    0xf70d: 0x0e4c,  # ์ (THANTHAKHAT)
    0xf70e: 0x0e4c,  # ์ (THANTHAKHAT)
    0xf710: 0x0e31,  # ั (MAI HAN-AKAT)
    0xf711: 0x0e4c,  # ์ (THANTHAKHAT)
    0xf712: 0x0e47,  # ็ (MAITAIKHU)
    0xf713: 0x0e48,  # ่ (MAI EK upper)
    0xf714: 0x0e49,  # ้ (MAI THO upper)
    0xf715: 0x0e4a,  # ๊ (MAI TRI upper)
    0xf716: 0x0e4b,  # ๋ (MAI CHATTAWA upper)
    0xf717: 0x0e4c,  # ์ (THANTHAKHAT upper)
    0xf718: 0x0e38,  # ุ (SARA U)
    0xf719: 0x0e39,  # ู (SARA UU)
    0xf71a: 0x0e3a,  # ฺ (PHINTHU)
}
...
def extract_pdf_text(path: str, page_spec: str | None = None) -> str:
    ...
    with pdfplumber.open(path) as pdf:
        ...
        for i in wanted:
            ...
            t = t.translate(THAI_PUA_MAP)
            # กรณีที่ 1 หน้ามีหลายภาคการศึกษา (เช่น AIT ที่ 1 หน้ามีทั้ง เทอม 1 และ เทอม 2)
            # แยกออกเป็นส่วนๆ เพื่อให้ LLM อ่านได้ครบทุกตาราง
            sem_splits = re.split(r'(?=\n\s*ปีที่\s*\d+\s*ภาคการศึกษาที่\s*\d+)', t)
            if len(sem_splits) > 1 and any('รหัสวิชา' in s for s in sem_splits[1:]):
                for sub_idx, s in enumerate(sem_splits):
                    if s.strip() and ('รหัสวิชา' in s or 'ภาคการศึกษา' in s or sub_idx > 0):
                        out.append(f"\n=== หน้า {i + 1} (ส่วนที่ {sub_idx + 1}) ===\n{s}")
            else:
                out.append(f"\n=== หน้า {i + 1} ===\n{t}")
    return "\n".join(out)
...
[1] สกัดทุกวิชาที่ปรากฏ ห้ามข้าม ห้ามหยุดกลางทาง
    - ดูให้ครบทุกหมวด: หมวดวิชาศึกษาทั่วไป, หมวดวิชาเฉพาะ, หมวดวิชาเลือกเสรี, สหกิจศึกษา
    - ⚠️ สำคัญมาก: ใน 1 หน้าเอกสารอาจมีหลายตารางหรือหลายภาคการศึกษา (เช่น มีทั้ง ภาคการศึกษาที่ 1 และ ภาคการศึกษาที่ 2 หรือมีตารางของหลายแขนงวิชาในหน้าเดียวกัน) ต้องสกัดให้ครบทุกตารางจนถึงบรรทัดสุดท้ายของหน้า ห้ามหยุดหลังจากจบตารางแรกเด็ดขาด
...
[2] ปี/ภาคการศึกษา — อ่านให้ดี ตรงนี้ผิดกันบ่อย
    ...
    - รายการในหมวดวิชาเลือก หรือวิชาที่อยู่นอกตารางแผนการศึกษา 4 ปี ซึ่งไม่ได้ระบุภาคเรียนตายตัว
        --> ใส่ year = 0 และ semester = 0 และ type = "เลือก"
...
[5] รหัสวิชา (code):
    - คัดลอกตามที่พิมพ์ เช่น "06026200"
    - หากในตารางมีคำว่า "หรือ" เช่น "06026259 หรือ 06026260" หรือ "06016481 หรือ 06016482" ให้คงรหัสตามข้อความนั้นไว้ (เช่น "06026259 หรือ 06026260")

[6] category ต้องเป็นหนึ่งใน 3 ค่านี้เท่านั้น:
    "หมวดวิชาศึกษาทั่วไป" | "หมวดวิชาเฉพาะ" | "หมวดวิชาเลือกเสรี"

[7] type ต้องเป็น "บังคับ" หรือ "เลือก" เท่านั้น

[8] ชื่อวิชาภาษาอังกฤษ ให้คัดลอกตามที่พิมพ์ รวมทั้งตัวพิมพ์ใหญ่
    ถ้าชื่อถูกตัดขึ้นบรรทัดใหม่ในเอกสาร ให้ต่อเป็นบรรทัดเดียวโดยเว้นวรรค 1 ครั้ง

[9] ⭐ แถว "ช่องวิชาเลือก" ที่ยังไม่ระบุวิชาเจาะจง
...
[10] ห้ามสร้างวิชาที่ไม่มีในเอกสาร ห้ามเติมวิชาที่ "น่าจะมี"
    ถ้าไม่แน่ใจว่าแถวนั้นเป็นวิชาหรือไม่ ให้ข้าม ดีกว่าใส่ข้อมูลผิด
    (แต่แถวช่องวิชาเลือกตามข้อ [9] ถือเป็นวิชา ต้องเก็บ)

=== ตัวอย่างการสกัด (Few-Shot Example) ===
ข้อความตัวอย่าง:
 ...
 9064xxxx วิชาเลือกหมวดวิชาศึกษาทั่วไป      3 (x-x-x)
...
{{
  "courses": [
    ...
    {{
      "code": "90641008",
      "name_th": "พื้นฐานทักษะการสื่อสารภาษาอังกฤษ",
      "name_en": "INTRODUCTION TO ENGLISH COMMUNICATION",
      "credits": "0(0-0-45)",
      "year": 1,
      "semester": 1,
    ...
    {{
      "code": "9064xxxx",
      "name_th": "วิชาเลือกหมวดวิชาศึกษาทั่วไป",
      "name_en": "GENERAL EDUCATION COURSES",
      "credits": "3(x-x-x)",
      "year": 1,
      "semester": 1,
      "category": "หมวดวิชาศึกษาทั่วไป",
...
def merge_chunks(chunks: list[dict]) -> dict:
    ...
    for ch in chunks:
        for c in ch.get("courses") or []:
            ...
            name = M.normalize(c.get("name_th"), "strict")
            y = int(str(c.get("year", 0) or 0))
            s = int(str(c.get("semester", 0) or 0))
            prereq = str(c.get("prerequisite") or "")
            note = str(c.get("note") or "")

            # ถ้ารหัสเป็นตัวเลข 8 หลักปกติ ใช้ code+name
            # ถ้าเป็น placeholder เช่น 06026xxx หรือ xxxxxxxx ต้องรวม year/sem เพื่อไม่ให้แถวซ้ำในเทอมเดียวกันหาย
            if len(code) == 8 and code.isdigit():
                key = f"{code}|{name}"
            else:
                key = f"{code}|{name}|{y}|{s}|{prereq}|{note}"

            if key in seen:
                existing = seen[key]
                ey = int(str(existing.get("year", 0) or 0))
                # หากตัวเดิม year=0 แต่อันใหม่มีปี/ภาคในแผนการศึกษาชัดเจน (year > 0) ให้อัปเดต
                if ey == 0 and y > 0:
                    seen[key] = c
                elif ey > 0 and y == 0:
                    pass  # คงตัวที่มีปี/ภาคชัดเจนไว้
                else:
                    n_dup += 1
            else:
                seen[key] = c
...
def extract_expected_credits(text: str) -> int | None:
    m = re.findall(r'(?:รวม|จำนวนหน่วยกิตรวม)\s+(\d{1,2})\b', text)
    if m:
        try:
            return int(m[-1])
        except ValueError:
...
def calc_extracted_credits(courses: list[dict]) -> int:
    tot = 0
    for c in courses:
        cr_str = str(c.get("credits") or "")
        m = re.search(r'(\d+)\s*(?:\(|$)', cr_str)
        if m:
            tot += int(m.group(1))
    return tot
...
def _text_to_json_chunked(md_pages: list[str]) -> dict:
    """แบ่งหน้าเป็นก้อน แล้วเรียก text LLM ทีละก้อน พร้อม Self-Correction ตรวจยอดหน่วยกิต"""
    chunks: list[dict] = []
    ...
    for ci in range(n_chunks):
        ...
        try:
            ...
            d = parse_json(raw)
            # ตรวจสอบว่าเป็นหน้ารายวิชาเลือกนอกแผนหรือไม่ (ไม่มีหัวตาราง 'ภาคการศึกษาที่')
            has_term = bool(re.search(r"ภาคการศึกษาที่|ปีที่\s*\d+", part_text))

            # Self-Correction: ถ้าเป็นตารางภาคการศึกษาที่มีระบุยอด 'รวม XX'
            if has_term:
                exp_cr = extract_expected_credits(part_text)
                act_cr = calc_extracted_credits(d.get("courses") or [])
                if exp_cr and act_cr < exp_cr:
                    print(f"      ⚠ ตรวจพบหน่วยกิตไม่ครบ: ได้ {act_cr}/{exp_cr} หน่วยกิต (ขาด {exp_cr - act_cr}) --> เรียก Self-Correction...")
                    refine_prompt = (
                        f"ในตารางแผนการศึกษานี้ระบุยอด 'รวม {exp_cr} หน่วยกิต' ชัดเจน "
                        f"แต่วิชาที่คุณสกัดมาได้รวมกันเพียง {act_cr} หน่วยกิต (ขาดไป {exp_cr - act_cr} หน่วยกิต) "
                        f"แสดงว่าคุณอ่านตกไปบางแถว โดยเฉพาะแถววิชาท้ายตารางที่ติดกับแถว 'รวม' หรือวิชาที่หน่วยกิตอยู่บรรทัดหัวกลุ่ม\n"
                        f"กรุณาตรวจทานตารางนี้ใหม่อย่างละเอียดถี่ถ้วน และสกัดทุกวิชาออกมาให้ครบถ้วนตาม schema:\n\n{part_text}"
                    )
                    try:
                        raw_refine = ollama_chat(
                            MODEL_TEXT,
                            [{"role": "system", "content": SYSTEM_PROMPT},
                        ...
                        d_refine = parse_json(raw_refine)
                        act_cr_refine = calc_extracted_credits(d_refine.get("courses") or [])
                        if act_cr_refine > act_cr:
                            print(f"      ✓ Self-Correction สำเร็จ: ได้เพิ่มเป็น {act_cr_refine}/{exp_cr} หน่วยกิต ({len(d_refine.get('courses') or [])} วิชา)")
                            d = d_refine
                    except Exception as err:
                        print(f"      (Self-Correction ข้าม: {err})")
...
def pipeline_text(pdf_path: str, page_spec: str | None) -> dict:
    ...
    n_all = len(re.findall(r"=== หน้า [^\n]+ ===", text))
    n_empty = n_all - len(pages_text)

    print(f"    ได้ข้อความ {len(text):,} ตัวอักษร จาก {len(pages_text)}/{n_all} หน้า/ส่วน")
